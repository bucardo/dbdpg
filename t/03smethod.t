...
SKIP: {
    ...
    $t='Statement handle method cancel() works as expected';
...

{
    ...
    while (my $row = $sth->fetchrow_arrayref) {
        $i++;
        $t="Statement handle method fetchrow_arrayref() returns correct value (row $i)";
        is (length($row->[1]), length(shift(@strings)), $t);
    }
    $sth->finish;
}
